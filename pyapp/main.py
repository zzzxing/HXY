...
from .models import (
    ...
    Badge, UserBadge, FeaturedItem, AIConversation,
)
...
def student_map(request: Request, db: Session = Depends(get_db)):
    ...
    evidence_site_ids = {s[0] for s in db.query(Evidence.site_id).filter(Evidence.activity_id == activity.id, Evidence.student_id == user.id).distinct().all()} if activity else set()
    return render(request, "student/map.html", user=user, activity=activity, sites=sites, evidence_site_ids=evidence_site_ids)
...
def student_site(site_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return render(request, "student/site_explore.html", user=user, site=site, hotspots=hotspots, evidences=evidences, guide_reply=request.session.pop("guide_reply", None), coach_reply=request.session.pop("coach_reply", None))
