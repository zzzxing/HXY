...
class SiteCompletion(Base):
    __tablename__ = "site_completions"
    __table_args__ = (UniqueConstraint("site_id", "student_id", name="uq_site_student_completion"),)
    id: Mapped[int] = mapped_column(primary_key=True)
    activity_id: Mapped[int] = mapped_column(ForeignKey("activities.id"))
    site_id: Mapped[int] = mapped_column(ForeignKey("activity_sites.id"))
    student_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
...
class ClassroomSession(Base):
    ...
    current_site_id: Mapped[int | None] = mapped_column(ForeignKey("activity_sites.id"))
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)


class ClassroomPresence(Base):
    __tablename__ = "classroom_presences"
    __table_args__ = (UniqueConstraint("session_id", "student_id", name="uq_session_student"),)
    id: Mapped[int] = mapped_column(primary_key=True)
    session_id: Mapped[int] = mapped_column(ForeignKey("classroom_sessions.id"))
    student_id: Mapped[int] = mapped_column(ForeignKey("users.id"))
    status: Mapped[str] = mapped_column(String(20), default="online")
    last_seen_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)


class ClassroomBroadcast(Base):
